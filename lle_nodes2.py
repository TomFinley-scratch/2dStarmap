...
class LLENode(Cumulator):
    ...
    def _stop_training(self):
        ...
        M = self.data
        N = M.shape[0] #sum(int(i) for i in self.importances) if self.importances else
        k = self.k
        r = self.r
        ...
        for row in range(N):
            if learn_outdim:
                ...
                # -----------------------------------------------
                M_Mi = M-M[row]
                args = numx.argsort((M_Mi**2).sum(1))
                halfk = k/2
                nbrs = numx.concatenate((args[1:halfk+1], args[-(k-halfk):]))
